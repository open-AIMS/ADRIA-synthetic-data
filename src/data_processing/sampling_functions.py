...
def sample_rand_radii(new_site_data, nrand_sites, n_gen):
    ...
    """
    rand_sites = np.unique(
        np.random.randint(0, len(new_site_data.lat), size=(1, nrand_sites))[0]
    )
    ...
    for rr in range(len(rand_radii)):
        ...
        if (
            rlat_temp[rand_sites[site][0]] < max_lat
            and rlat_temp[rand_sites[site][0]] > min_lat
        ):
            rand_lats.append(rlat_temp[rand_sites[site][0]])
        rlong_temp = longs[rand_sites[site]] + rand_radii[rr] * np.sin(rand_theta[rr])

        if (
            rlong_temp[rand_sites[site][0]] < max_long
            and rlong_temp[rand_sites[site][0]] > min_long
        ):
